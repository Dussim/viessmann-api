...
fun <E> ofCurrentEleven(
    ...
    if (!result1.isInvalid && !result2.isInvalid && !result3.isInvalid && !result4.isInvalid &&
        ...
        !result9.isInvalid && !result10.isInvalid && !result11.isInvalid
    ) {
        return Valid
    }
    val capacity =
        result1.size + result2.size + result3.size + result4.size + result5.size +
            result6.size + result7.size + result8.size + result9.size + result10.size + result11.size
    if (capacity == 1) {
        when {
...
fun <E> ofAlternativeEleven(
    ...
    val mask =
        result1.invalidFlag or (result2.invalidFlag shl 1) or (result3.invalidFlag shl 2) or
            ...
            (result10.invalidFlag shl 9) or (result11.invalidFlag shl 10)
