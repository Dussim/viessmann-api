...
fun <E> ofOldNine(
    ...
    if (!result1.isInvalid && !result2.isInvalid && !result3.isInvalid &&
        !result4.isInvalid && !result5.isInvalid && !result6.isInvalid &&
        !result7.isInvalid && !result8.isInvalid && !result9.isInvalid
    ) {
        return Valid
    }
    val capacity =
        result1.size + result2.size + result3.size + result4.size + result5.size +
            result6.size + result7.size + result8.size + result9.size
    if (capacity == 1) {
        when {
...
fun <E> ofCurrentNine(
    ...
    val mask =
        result1.invalidFlag or (result2.invalidFlag shl 1) or (result3.invalidFlag shl 2) or
            ...
            (result7.invalidFlag shl 6) or (result8.invalidFlag shl 7) or (result9.invalidFlag shl 8)
