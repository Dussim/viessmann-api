...
## Integration
The processor is integrated into the build process of modules that define or consume features (e.g., `:api:dto` and `:api:yaml-generator`). It runs during the `kspCommonMainKotlinMetadata` task (or similar metadata tasks) to ensure that generated code is available for all multiplatform targets.
