...
These interface definitions can be either written manually or generated from YAML API specifications using the `xyz.dussim.generate.features.json` and `xyz.dussim.generate.features` Gradle plugins (configured in modules like `:api:feature:definitions`).
